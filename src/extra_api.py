...
def get_transaction_sum(transaction: dict) -> float:
    ...
    if target_currency == "RUB":
        ...
        response = requests.get(f'https://v6.exchangerate-api.com/v6/{key}/latest/{target_currency}')
        data = response.json()

        return float(data['conversion_rates']['RUB'] * float(transaction["operationAmount"]["amount"]))
